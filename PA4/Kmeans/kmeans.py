...
class KMeansClassifier():
    ...
    def fit(self, x, y, centroid_func=get_lloyd_k_means):
        ...
        for i in range(len(membership)):
            if y[i] not in member_votes[membership[i]]:
                member_votes[membership[i]][y[i]] = 1
            else:
                member_votes[membership[i]][y[i]]+=1
        centroid_labels = []
    ...
    def predict(self, x):
        ...
        # - return labels
        distances = np.sum((x - self.centroids[:, np.newaxis]) ** 2, axis=2)
        membership = np.argmin(distances, axis=0)
...
def transform_image(image, code_vectors):
    ...
    distances = np.sum((image - code_vectors[:, np.newaxis]) ** 2, axis=2)
    membership = np.argmin(distances, axis=0)
    new_im = code_vectors[membership].reshape(R, G, B)
